...
<CommandPanel
  title="Full local test pipeline"
  commands={[
...
<Callout title="E2E requires running local Supabase" tone="warning">
  E2E tests connect to a real local Supabase instance. Always run `supabase start` and `supabase db reset` before running `pnpm e2e`. If tests fail with auth errors, re-run `supabase db reset` to restore seeded users.
</Callout>
