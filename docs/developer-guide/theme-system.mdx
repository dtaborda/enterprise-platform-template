...
> **⚠️ Both files must have identical semantic key sets:** `light.json` and `dark.json` must contain exactly the same semantic color keys. The build script verifies this and fails if keys differ between files. Missing a key in one file will cause a build error.
...
> **🚨 Never edit generated files manually:** `theme-generated.css` and `tokens/index.ts` are generated by the build script. Edits will be overwritten on the next `build:theme` run. Always edit the JSON source files and regenerate.
