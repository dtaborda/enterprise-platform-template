---
title: "Theme System"
description: "JSON-configurable theme architecture — tokens, build pipeline, light/dark switching, and how to extend the design system."
owner: "Engineering"
...
This document explains the Enterprise Platform's JSON-configurable theme system — how design tokens are defined, validated, generated as CSS variables, and consumed by components.

## Scope

- **Included**: Theme architecture, JSON structure, token references, CSS generation, light/dark switching, build pipeline, schema validation, token-to-CSS mapping, extending the theme
- **Excluded**: Per-component styling patterns (see [Design Components](./design-components.mdx) skill), design token values catalogue
...
Each theme file (`light.json`, `dark.json`) has three top-level sections: `metadata`, `foundations`, and `layout`.
...
<Callout title="Both files must have identical semantic key sets" tone="warning">
  `light.json` and `dark.json` must contain exactly the same semantic color keys. The build script verifies this and fails if keys differ between files. Missing a key in one file will cause a build error.
</Callout>
...
<Callout title="Never edit generated files manually" tone="danger">
  `theme-generated.css` and `tokens/index.ts` are generated by the build script. Edits will be overwritten on the next `build:theme` run. Always edit the JSON source files and regenerate.
</Callout>
...
<CommandPanel
  title="Edit and rebuild theme"
  commands={[
    "# 1. Edit the value in light.json and/or dark.json",
    "# 2. Rebuild",
...
1. Add the primitive color in `colors.primitive` in **both** `light.json` and `dark.json`.
2. Add the semantic reference in `colors.semantic` in both files.
...
|------|---------|-------------|
| `defaultMode` | `"dark"` | SSR default — must match `data-theme` on `<html>` |
| `storageKey` | `"enterprise-theme-mode"` | localStorage key for persisted preference |

`suppressHydrationWarning` prevents the React hydration warning when the client hydrates with a different mode than the server-rendered HTML.
...
1. `ThemeProvider` sets the `data-theme` attribute on `<html>`.
2. The CSS selector `[data-theme="dark"]` overrides the `@theme` color variables.
3. Tailwind utilities (`bg-primary`, `text-foreground`) automatically pick up the new values.
...
7. Writes resolved JSONs to `src/themes/resolved/` (for debugging).
...
`build:theme` runs **before** `typecheck` and `build` in the Turborepo pipeline. It is cached based on `src/themes/*.json` inputs — if no JSON files changed, the cache is hit and code generation is skipped.
...
The schema includes an optional `tenantId` field. The planned V2 architecture will:

1. Store theme JSON per tenant in Supabase.
2. Load at request time via middleware.
3. Generate CSS per tenant (cached at edge).
4. No V1 code changes needed — the JSON structure and CSS variable names are stable and forward-compatible.
