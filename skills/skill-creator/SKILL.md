...
| Project-specific | `{project}-{component}` | `enterprise-api`, `enterprise-ui` |
| Testing skill | `{project}-test-{component}` | `enterprise-test-api` |
...
Skills follow a two-directory convention:

| Directory | Purpose | Contains |
|-----------|---------|----------|
| `skills/{name}/` | **Source of truth** | `SKILL.md`, `assets/`, `references/` |
| `.agents/skills/{name}` | **Discovery symlink** | Symlink to `../../skills/{name}` |

### Rules

1. **ALWAYS create the skill in `skills/{name}/SKILL.md`** — this is the canonical location
2. **ALWAYS create a symlink** in `.agents/skills/` pointing to the source:
   ```bash
   ln -s ../../skills/{name} .agents/skills/{name}
   ```
3. **NEVER create a real directory inside `.agents/skills/`** — only symlinks live there
4. **NEVER write files directly to `.agents/skills/`** — always write to `skills/` first

### Verification

After creating a skill, verify the symlink is correct:
```bash
ls -la .agents/skills/{name}
# Should show: .agents/skills/{name} -> ../../skills/{name}
```
