...
description: >
  ...
  Trigger: When writing Playwright E2E tests (Page Object Model, selectors, MCP exploration workflow). For project-specific UI conventions under ui/e2e, also use the project E2E patterns reference.
license: Apache-2.0
metadata:
...
**enterprise-platform project structure** (canonical):
```
ui/e2e/
├── helpers/
│   └── auth.ts                   # Login helper (seed defaults)
├── {feature}/
│   ├── {feature}-page.ts         # Page Object Model
│   └── {feature}.spec.ts         # ALL tests here (NO separate files!)
```

**Prerequisites to run E2E tests locally:**
```bash
supabase start    # Supabase local must be running (migrations + seed)
pnpm build        # Production build required (tests use pnpm start, NOT dev)
pnpm e2e          # Run all tests
```

**Key patterns for this project:**
- Use `[role="dialog"][data-state="open"]` for shadcn/Radix dialogs
- Use `button[aria-haspopup="menu"]` for DropdownMenuTrigger
- shadcn Select uses `role="combobox"`, NOT `getByLabel`
- Use `{ exact: true }` for badge text assertions to avoid matching toasts
- Use `test.describe.serial` when tests share database state (create → update → delete)
- NEVER use `networkidle` — use element visibility waits instead
...
**Guidelines:**
- Check `ui/e2e/` for existing page objects first
...
npx playwright test ui/e2e/login/      # Run specific folder
pnpm e2e                               # Project shortcut
```
